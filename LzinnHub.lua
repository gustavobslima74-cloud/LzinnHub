...
-- TESTE (COMBO)
TestTab:CreateSection("Sincronização")
...
TestTab:CreateToggle({
    ...
    Callback = function(v) 
        ...
        autoSelect = v
        followEnabled = v
        -- O combo agora controla apenas a própria variável interna
        Rayfield:Notify({
            Title = "Modo Combo",
...
-- NOVO LOOP DE ATAQUE (CORREÇÃO DE PARADA)
task.spawn(function()
    while true do
        -- Verifica as duas condições para atacar
        if autoAttackV1 or comboEnabled then
            VIM:SendMouseButtonEvent(0, 0, 0, true, game, 0)
            task.wait(0.01)
            VIM:SendMouseButtonEvent(0, 0, 0, false, game, 0)
            task.wait(0.5) -- Delay de 500ms
        else
            -- Se desligado, espera um pouco e checa de novo sem atacar
            task.wait(0.1)
        end
    end
end)
